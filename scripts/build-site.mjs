...
import {
  renderJapaneseHome,
  renderJapaneseAbout,
  renderJapaneseContact
} from "../site/ja-pages.mjs";
...
for (const lang of langs) {
  ...
  const home = lang === "ja" ? renderJapaneseHome(options) : renderHome(lang, options);
  const about = lang === "ja" ? renderJapaneseAbout(options) : renderAbout(lang, options);
  const contact = lang === "ja" ? renderJapaneseContact(options) : renderContact(lang, options);

  await emit(`${lang}/index.html`, home);
  ...
  await emit(`${lang}/about/index.html`, about);
  await emit(`${lang}/contact/index.html`, contact);
}