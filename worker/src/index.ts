...
import { handleRegistryRequest } from "./registry/routes";
...
async function readJsonBody(request: Request): Promise<unknown> {
  ...
  try { return JSON.parse(text); } catch { throw new InvalidRequestError(); }
}
...
export async function handleRequest(request: Request, env: RuntimeEnv): Promise<Response> {
  ...
  if (url.pathname.startsWith("/v1/admin/projects") || url.pathname.startsWith("/v1/public/projects")) {
    try {
  ...
  try {
    ...
    if (url.pathname === "/v1/chat") {
      let chatRequest;
      try { chatRequest = parseChatRequest(raw); } catch { throw new InvalidRequestError(); }
      if (!(await rateLimit(env.CHAT_RATE_LIMITER, `chat:${chatRequest.sessionId}`))) return jsonResponse({ error: "rate_limited" }, 429, origin, env);
      try {
        const result = await generateSalesReply(env, chatRequest);
        return jsonResponse(result, 200, origin, env);
      } catch (error) {
        if (error instanceof AiServiceError) return jsonResponse({ error: "ai_unavailable", requestId }, 503, origin, env);
        throw error;
      }
    }

    let leadRequest;
    try { leadRequest = parseLeadSubmission(raw); } catch { throw new InvalidRequestError(); }
    if (!(await rateLimit(env.LEAD_RATE_LIMITER, `lead:${leadRequest.sessionId}`))) return jsonResponse({ error: "rate_limited" }, 429, origin, env);
    const result = await submitLead(env, leadRequest);
    if (!result.ok) return jsonResponse(result, 502, origin, env);
    return jsonResponse(result, 200, origin, env);
  } catch (error) {
    if (error instanceof BodyTooLargeError) return jsonResponse({ error: "payload_too_large" }, 413, origin, env);
    if (error instanceof InvalidRequestError) return jsonResponse({ error: "invalid_request" }, 400, origin, env);
